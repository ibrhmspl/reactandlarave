...
export const postStoreProduct =  (data)=> {
  let token = localStorage.getItem('token');
  let user = JSON.parse(localStorage.getItem('user')); 
  return axios
    .post('http://127.0.0.1:8000/api/store_product', {product_id:data.productId, price:data.price, stock:data.stock, store_id:user.user.store_id},{
      headers: { Authorization: `Bearer ${token}` }
    })
    .then((response)=>{
